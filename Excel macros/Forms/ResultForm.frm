VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} ResultForm 
   Caption         =   "Scoring"
   ClientHeight    =   7908
   ClientLeft      =   -133
   ClientTop       =   -469
   ClientWidth     =   12229
   OleObjectBlob   =   "ResultForm.frx":0000
...
Private Sub CommandButton4_Click()
    ' Ensure the NewChildBlockForm is open
    CommandButton1_Click
    ' Set the labelBlockID to the value of TextBox1
    NewChildBlockForm.LabelBlockID.Caption = Me.TextBox1.value
    NewChildBlockForm.LabelMarker.Caption = Me.ListBox1.value
...
Private Sub CommandButton3_Click()
    ...
    Dim markerUsedCol As Range
    Dim markerCell As Range
    Dim MarkerList() As String
    Dim j As Long
    Dim updatedMarkerList As String
    Dim isUpdated As Boolean

    ' Retrieve the Block Name from TextBox1
    BlockName = Me.TextBox1.value

    ' Check if the Block Name is provided
    If BlockName = "" Then
        MsgBox "Please enter a Block Name.", vbExclamation
        Exit Sub
    End If
    ...
    ' Retrieve the Marker from ListBox1
    If Me.ListBox1.ListIndex = -1 Then
        MsgBox "Please select a Marker.", vbExclamation
        Exit Sub
    End If
    Marker = Me.ListBox1.List(Me.ListBox1.ListIndex)

    ' Set the worksheet and table containing blocks
    Set ws = BlocksWS
    Set BlocksTable = ws.ListObjects(BlocksTableName)

    ' Determine the correct column name for the block type
    ...
    If blockRow = -1 Then
        MsgBox "The Block Name " & BlockName & " was not found.", vbExclamation
        Exit Sub
    End If

    ' Get the "Marker Used" column
    Set markerUsedCol = BlocksTable.ListColumns(MarkerUsedColName).DataBodyRange
    Set markerCell = markerUsedCol.Cells(blockRow - BlocksTable.DataBodyRange.row + 1)

    ' Split the markers into a list
    MarkerList = Split(markerCell.value, "|")
    isUpdated = False

    ' Loop through the list to find and replace "(in Review)" with "[REJECTED]"
    For j = LBound(MarkerList) To UBound(MarkerList)
        If InStr(1, MarkerList(j), Marker, vbTextCompare) > 0 Then
            MarkerList(j) = Replace(MarkerList(j), "(in Review)", "[REJECTED]")
            isUpdated = True
        End If
    Next j

    ' If the marker was updated, join the list back and save it to the cell
    If isUpdated Then
        updatedMarkerList = Join(MarkerList, " | ")
        markerCell.value = updatedMarkerList
        MsgBox "Marker '" & Marker & "' updated to '[REJECTED]' in the Block '" & BlockName & "'.", vbInformation
    Else
        MsgBox "The selected marker '" & Marker & "' was not found in '(in Review)' state.", vbExclamation
    End If
End Sub
...
Private Sub CommandButton1_Click()
    ...
    ' Initialize variables for name generation
    currentDate = Format(Date, "yyyymmdd") ' Use YYYYMMDD format for the date
    ...
    For i = 0 To Me.ListBoxScoring.ListCount - 1
        If Me.ListBoxScoring.Selected(i) Then
            ...
            If value = "" Then
                If isTMA Then
                    MarkerScoresDict(CurrentMarker).Add selectedItem & "{Multiplex}"
                Else
                    MarkerScoresDict(CurrentMarker).Add selectedItem
                End If
            Else
                If isTMA Then
                    MarkerScoresDict(CurrentMarker).Add selectedItem & "{Multiplex}:" & value
                Else
